...
`dutchie_proxy.gs` (Dutchie + QuickBooks proxy). Frontend: `index.html`, deployed via **Netlify**. Its
...
via clasp (backend) + Netlify (frontend). ⚠️ No shared login yet — open URL = access (Phase-2 item).
Current app version **v38**; the Version History list in `index.html` is currently **hardcoded** (to be
repointed at GX Core `version_history`). GX Core already holds Sales' imported v1–v39 history.
